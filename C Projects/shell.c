...
int my_contains(command_t* p_cmd, char* symbol)
{
  int i;
  
  for (i = 0; i < p_cmd->argc; i++) {
    if (my_strequal(p_cmd->argv[i],symbol))
      return i;
  }
...
int execute(command_t* p_cmd)
{
  ...
  int fds[2];   // for pipe
  int copy_file_marker; //position of marker to copy input into file, returns -1 if there is not one 
  int outfile;
  int ampersand;
  int cpid1, cpid2;


  if ((pp = my_contains(p_cmd, "|")) != -1) {   // if we find a pipe symbol...
  ...
  if ((copy_file_marker = my_contains(p_cmd, ">")) != -1){
	p_cmd->argv[copy_file_marker] = NULL;
	outfile = open(p_cmd->argv[copy_file_marker+1], O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP);
	if( outfile == -1){
  ...
  if (((ampersand = my_contains(p_cmd, "&")) != -1) && (p_cmd->argv[ampersand+1]==NULL)){
	  p_cmd->argv[ampersand] = NULL;
